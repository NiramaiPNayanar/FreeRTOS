...
/*-----------------------------------------------------------*/
void vTaskGeneric(void* pvParameters)
{
...
void vEDFSchedulerTask(void* pvParameters)
{
    for (;;)
    {
        // Sort tasks by next_deadline (simple selection sort for 4 tasks)
        ...
        // Assign priorities: idx[0] gets highest, idx[3] gets lowest
...
int main(void)
{
    ...
    xWindowsKeyboardInputThreadHandle = CreateThread(
        NULL, 0, prvWindowsKeyboardInputThread, NULL, 0, NULL);
    ...
    edfTasks[0].name = "Task1";
    xTaskCreate(vTaskGeneric, "Task1", 1000, &edfTasks[0], edfPriority[3], &edfTasks[0].handle);

    edfTasks[1].period = pdMS_TO_TICKS(500);
    edfTasks[1].next_deadline = xTaskGetTickCount() + edfTasks[1].period;
    edfTasks[1].name = "Task2";
    xTaskCreate(vTaskGeneric, "Task2", 1000, &edfTasks[1], edfPriority[3], &edfTasks[1].handle);

    edfTasks[2].period = pdMS_TO_TICKS(750);
    edfTasks[2].next_deadline = xTaskGetTickCount() + edfTasks[2].period;
    edfTasks[2].name = "Task3";
    xTaskCreate(vTaskGeneric, "Task3", 1000, &edfTasks[2], edfPriority[3], &edfTasks[2].handle);

    edfTasks[3].period = pdMS_TO_TICKS(1200);
    edfTasks[3].next_deadline = xTaskGetTickCount() + edfTasks[3].period;
    edfTasks[3].name = "Task4";
    xTaskCreate(vTaskGeneric, "Task4", 1000, &edfTasks[3], edfPriority[3], &edfTasks[3].handle);

    /* Create EDF scheduler management task */
    xTaskCreate(vEDFSchedulerTask, "EDFSched", 1000, NULL, edfPriority[0] + 1, NULL);
